...
Updated by Louise Brown 8 August 2022
...
int getStringRS232(int cport_nr, unsigned char * buf, unsigned char * buf2, int * stringRead);
//int getStringRS232(int cport_nr, char * buf, char * combinedBuf2, int * stringRead);
...
{
  int i=0, j=0, numRead;
  int cport_nr=3;        /* Set this to COM port number - 1 */
  int bdrate = BAUD_RATE;       
  ...
  printf("Serial port is set to COM%d, is this correct?  Terminate program (ctrl-C) if not.\n", cport_nr+1);
